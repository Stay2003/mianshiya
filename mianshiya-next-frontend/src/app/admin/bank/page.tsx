...
import type {
  ...
  QuestionBankQueryRequest as APIQuestionBankQueryRequest,
} from "@/api/typings"
...
const QuestionBankAdminPage: React.FC = () => {
  ...
  // 当前题库点击的数据
  const [currentRow, setCurrentRow] = useState<APIQuestionBank>()
   ...
   * @param row
   */
  const handleDelete = (row: APIQuestionBank) => {
    Modal.confirm({
      title: "确认删除",
   ...
   * 表格列配置
   */
  const columns: ProColumns<APIQuestionBank>[] = [
    {
      title: "id",
      dataIndex: "id",
      valueType: "text",
        ...
        <ProTable<APIQuestionBank>
            ...
            request={async (params, sort, filter) => {
              ...
              const { data, code } = await listQuestionBankByPageUsingPost({
                ...
                sortOrder,
                ...filter,
              } as APIQuestionBankQueryRequest)
